# Sotto: Product and Architecture

## 1. Product summary

Sotto is a local-first, voice-first mobile idea vault. It is designed for the moment when a person has a useful thought but does not have time to title, tag, or organize it. The shortest successful journey is:
...
- Explain what Sotto does and that content is stored locally.
...
Sotto starts as one Expo application with no required product backend, hosted database, account system, or server worker.
...
The earlier build at `the original prototype` is a behavioral reference. It demonstrates useful approaches such as local SQLite persistence, SecureStore-backed BYOK credentials, filesystem audio, provider adapters, resumable jobs, report exports, and idea-bound discussions.
