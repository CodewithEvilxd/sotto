...
for (const file of sourceFiles) {
  ...
  if (!header.includes("@author Nishant Gaurav"))
    failures.push(`${file}: missing @author Nishant Gaurav`);
  if (!header.includes("@license MIT"))
    failures.push(`${file}: missing @license MIT`);
  if (
    /implementation for (?:Hmmmidea|Sotto)|invoked collaborators own/u.test(
      source,
    )
  ) {
