...
const EXPORT_DIRECTORY = "sotto-share";
const ARTIFACT_NAME = /^sotto-[a-z0-9-]+\.(?:json|pdf)$/u;
...
export function sanitizeExportFilename(value: string, fallback = "sotto") {
  const stem = value
    .normalize("NFKD")
    .replace(/[\u0300-\u036f]/gu, "")
...
function jsonArtifact(bundle: NonSecretExportBundle) {
  return writeCacheText(
    artifactFilename("sotto-data-export", "json"),
...
async function pdfArtifact(request: PdfExportRequest): Promise<ExportArtifact> {
  const fileName = artifactFilename(
    `sotto-${request.capture.title ?? "idea"}`,
    "pdf",
  );
...
export async function shareExportArtifact(artifact: ExportArtifact) {
  ...
  await Sharing.shareAsync(artifact.uri, {
    dialogTitle: "Share Sotto export",
