...
export function researchProviderDescription(providerId: string, model: string) {
  if (!supportsProviderResearch(providerId, model))
    return "This model has no enabled research path in Sotto.";
  if (providerId === "groq")
    return model.trim().toLowerCase() === "groq/compound-mini"
      ? "Groq Compound Mini may run one server-side web search, then returns the result and automatic citations in one response. Sotto allows only its web-search tool."
      : "Groq Compound decides when to run server-side web searches and may make several tool calls. Sotto allows only its web-search tool and saves the returned sources.";
