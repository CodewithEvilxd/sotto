...
function processingStatus(capture: CaptureRecord, jobs: readonly JobRecord[]) {
  ...
  if (!job || job.status === "queued") {
    return {
      title,
      body: "Saved on this device and waiting to start. Keep Sotto open so processing can continue.",
    };
  }
  ...
  if (job.status === "retry-wait") {
    return {
      ...
      body: `Attempt ${job.attempts} of ${job.maxAttempts} did not finish. The next retry is scheduled for ${formatStatusTime(job.runAfter)} while Sotto is open.`,
    };
  }
  return {
    title,
